...
export function computeBadgeProgress(
  ...
  stats: { wins: bigint; losses: bigint; totalGames: bigint; streak: bigint; bestStreak: bigint } | null,
  matchHistory: DayWithLog[]
): BadgeProgress {
  const criteria: BadgeCriteria = definition.criteria;

  const safeStats = stats ?? { wins: 0n, losses: 0n, totalGames: 0n, streak: 0n, bestStreak: 0n };

  switch (criteria.__kind__) {
    case 'totalWins': {
      const required = Number(criteria.totalWins);
      const current = Number(safeStats.wins);
      return { current, required, percentage: required > 0 ? Math.min(100, Math.round((current / required) * 100)) : 0 };
    }
    case 'winsStreak': {
    ...
    case 'winPercentage': {
      ...
      const current = total > 0 ? Math.round((Number(safeStats.wins) / total) * 100) : 0;
      return { current, required, percentage: required > 0 ? Math.min(100, Math.round((current / required) * 100)) : 0 };
    }
 ...
 */
export function getBadgeProgressLabel(definition: BadgeDefinition, progress: BadgeProgress): string {
