...
export default function ReplyComposer({ parentId, onSuccess, onCancel }: ReplyComposerProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-2 pl-8 pt-2 min-w-0">
      ...
      <div className="flex gap-2 min-w-0 items-center">
        ...
        />
        <Button type="submit" disabled={(!content.trim() && !selectedImage) || isPending} size="sm" className="flex-shrink-0">
          <Send className="h-3 w-3" />
        </Button>
        {onCancel && (
          <Button type="button" variant="ghost" size="sm" onClick={onCancel} disabled={isPending} className="flex-shrink-0">
            Cancel
          </Button>
        )}
      </div>
